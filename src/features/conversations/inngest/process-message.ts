...
export const processMessage = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    // Run the coding agent in a durable step so it executes exactly once
    // regardless of how many times Inngest replays the function body.
    // We return only the final text to stay well under the 2 MB step-result limit.
    const assistantResponse = await step.run("run-coding-agent", async () => {
      const result = await generateText({
