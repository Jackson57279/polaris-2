...
This project uses Clerk Billing for subscriptions, with a Convex-backed wallet ledger for internal credits.

Routes:
- Pricing table (prebuilt Clerk component)
  - `GET /pricing`
- Customer portal (Clerk user profile billing area)
  - `GET /customer-portal`
...
Convex billing tables:
- `payments`, `subscriptions`, `wallets`, `wallet_ledger`, `billing_webhook_events`

Convex billing functions:
- `convex/billing.ts`: `upsertPayment`, `upsertSubscription`, `recordWalletLedger`, `recordWebhookEvent`

Environment variables:
- `CLERK_WEBHOOK_SIGNING_SECRET=whsec_xxx`
- `POLARIS_CONVEX_INTERNAL_KEY=generate_a_random_secret`

Wallet ledger request body example:
```json
{
...
Notes:
- Clerk Billing powers subscription state and plan checks via Clerk auth (`has({ plan: "pro" })`).
- Webhook payloads are verified with Clerk’s signing secret and stored in Convex for audit/debugging.
- Wallet credits are Convex-only and keyed by `clerkUserId`.
